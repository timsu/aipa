...
export default function QuestionAttributes({ question, divRef }: Props) {
  const questionId = useStore(editFormStore.selectedQuestionId);
  ...
  return (
    <div
      ...
      className="select-none rounded-md bg-slate-800 text-white border p-4 shadow-lg w-full 
        max-w-sm lg:max-w-md min-h-[50px]"
      style={styles.popper}
      {...attributes.popper}
    >
      <div className="flex items-center -mt-2 -mx-4 px-4 py-1 mb-4 border-b">
        <div className="flex-1">{questionTitle(question)}</div>
        <div className="cursor-pointer ml-5" onClick={() => close()}>
          <XMarkIcon className="h-5 w-5" />
        </div>
      </div>
      <div className="grid grid-cols-1x2 gap-4 items-center">
        {isTextQuestion(question.type) && <TextFieldOptions question={question} />}
        {!isTextQuestion(question.type) && <TitleOptions question={question} />}
        {isChoiceQuestion(question.type) && <ChoiceFieldOptions question={question} />}
